...
class View {
    ...
    create = () => {
        let content = `<div class="between"></div>
...
class Model {
    constructor (el) {
        ...
        this.between = el.querySelector('.between')
        this.button1 = el.querySelector('.btn1')
        this.button2 = el.querySelector('.btn2')
        this.size = el.querySelector('.range__size')          
    ...
    movedSlider = (el1, el2) => {
        ...
        el1.addEventListener('mousedown', () => {
            ...
            const moveBtn = e => {
                ...
                if (bL > this.slider.offsetWidth - btnWidth/2) {bL = this.slider_inner.offsetWidth - btnWidth/2}
                else if (bL < 0 - btnWidth/2 ) {bL = 0 - btnWidth/2 }
                el1.style.marginLeft = bL + 'px'
            
                let bR = coorBtn2 - coorSlider
        
                if (bR > bL) {this.between.style.marginLeft = bL + btnWidth/2 + 'px'}
                else {this.between.style.marginLeft = el2.style.marginLeft + btnWidth/2 + 2}
                this.between.style.width = Math.abs(bR - bL) + 'px'

                bL = (bL + btnWidth/2) *56.392 
                bR = (bR + btnWidth/2) *56.392
                let n;
                if (el1 === this.button1){if (bL > bR) {n = bL; bL = bR; bR = n}}
                else {if (bL < bR) {n = bL; bL = bR; bR = n}}
                if (el1 === this.button1) {this.size.innerHTML = parseInt(bL) + ' &#8381 - ' + parseInt(bR) + ' &#8381'}
                else {this.size.innerHTML = parseInt(bR) + ' &#8381 - ' + parseInt(bL) + ' &#8381'}
            }
